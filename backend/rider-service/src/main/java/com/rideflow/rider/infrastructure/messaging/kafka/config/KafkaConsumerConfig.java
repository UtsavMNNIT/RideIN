...
/**
 * Consumer-side Kafka wiring for the ride-projection listener. Bad data →
 * DLQ on first try; transient infra faults → backoff then DLQ after the budget.
 *
 * <p>The DLQ producer beans are scoped to the consumer path and named distinctly
 * so they don't collide with the outbox relay's {@code KafkaTemplate<String,String>}.
 */
@Configuration
