...
/**
 * Drains the transactional outbox ({@code matching.outbox}) to Kafka.
 *
 ...
 * dedupe on the envelope {@code eventId}. We never lose an event; duplicates are
 * harmless.
 *
 ...
 * per-ride order. A row that keeps failing past {@code max-attempts} drops out
 * of the query and is left for an operator (a poison-event alert fires on the
 * {@code attempt_count} metric).
 *
 * <p><b>Multi-pod.</b> Several pods may drain concurrently and double-send;
 * consumer dedupe absorbs it. Switch to {@code SELECT … FOR UPDATE SKIP LOCKED}
 * if the redundant traffic ever matters.
 */
@Component
public class OutboxRelay {
    ...
    public OutboxRelay(OutboxJpaRepository outbox,
                       ...
                       @Value("${rideflow.dispatch.outbox.batch-size}")   int batchSize,
                       @Value("${rideflow.dispatch.outbox.max-attempts}") int maxAttempts,
                       @Value("${rideflow.dispatch.outbox.send-timeout-ms:5000}") long sendTimeoutMs,
        ...
        this.publishedCounter = registry.counter("dispatch.outbox.published");
        this.failedCounter    = registry.counter("dispatch.outbox.send_failed");
    }

    @Scheduled(fixedDelayString = "${rideflow.dispatch.outbox.poll-interval-ms}")
    @Transactional
    public void drain() {
        ...
        for (OutboxEvent event : batch) {
            try {
                ...
                // Record + stop. Do NOT rethrow (would roll back rows we already
                // marked published this batch). Retried from here next tick.
