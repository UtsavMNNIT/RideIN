...
/**
 * JPA mapping for {@code matching.processed_events} — consumer-side idempotency.
 * Composite key {@code (event_id, consumer_group)}: the same envelope handled by
 * two different consumer groups is two distinct rows, but a redelivery to the
 * same group collides on the PK.
 */
@Entity
@Table(name = "processed_events", schema = "matching")
