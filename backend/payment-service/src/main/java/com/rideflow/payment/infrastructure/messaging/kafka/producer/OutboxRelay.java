...
/**
 * Drains the transactional outbox ({@code payment.outbox}) to Kafka.
 * At-least-once: a row is sent then marked published in the same transaction; a
 * crash between re-sends next tick (consumers dedupe on {@code eventId}).
 */
@Component
public class OutboxRelay {
    ...
    public OutboxRelay(OutboxJpaRepository outbox,
                       ...
                       @Value("${rideflow.payment.outbox.batch-size}")   int batchSize,
                       @Value("${rideflow.payment.outbox.max-attempts}") int maxAttempts,
                       @Value("${rideflow.payment.outbox.send-timeout-ms:5000}") long sendTimeoutMs,
        ...
        this.publishedCounter = registry.counter("payment.outbox.published");
        this.failedCounter    = registry.counter("payment.outbox.send_failed");
    }

    @Scheduled(fixedDelayString = "${rideflow.payment.outbox.poll-interval-ms}")
    @Transactional
    public void drain() {
